...
export const UsersView: React.FC<UsersViewProps> = ({ onNavigate }) => {
  ...
  const handleOpenAddModal = (prefillStaff?: Staff) => {
    ...
    if (prefillStaff) {
      ...
      setAddForm({
        ...
        username: rawUser,
        email: prefillStaff.email || `${rawUser}@example-school.ac.ke`,
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      const fallbackEmail =
        addForm.email.trim() ||
        `${(addForm.username || addForm.fullName).toLowerCase().replace(/[^a-z0-9]/g, '.')}@example-school.ac.ke`;
        ...
        <form onSubmit={handleCreateUser} className="space-y-4">
                ...
                onChange={(e) => {
                  ...
                  if (found) {
                    ...
                    setAddForm((prev) => ({
                      ...
                      username: cleanName,
                      email: found.email || `${cleanName}@example-school.ac.ke`,
                      phone: found.phone || '',
                ...
                onChange={(e) => {
                  ...
                  setAddForm((prev) => {
                    ...
                    return {
                      ...
                      fullName: val,
                      username: autoUser,
                      email: !prev.email || prev.email.includes('@example-school.ac.ke') ? `${autoUser}@example-school.ac.ke` : prev.email,
                    };
                  });
                }}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                type="email"
                placeholder="e.g. cmutua@example-school.ac.ke"
                value={addForm.email}
          ...
          <div className="space-y-4">
            <div className="border-2 border-blue-900 rounded-2xl p-4 bg-gradient-to-b from-blue-50/50 to-white shadow-sm space-y-3">
              <div className="flex items-center justify-between border-b border-slate-200 pb-2.5">
                <div>
                  <h4 className="font-extrabold text-blue-900 text-sm">{school?.name || 'Primary School ERP'}</h4>
