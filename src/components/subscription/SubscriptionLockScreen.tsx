      ...
      >
        <div className="space-y-4 text-xs">
          <p className="text-slate-600">
            Please reach out to the system owner or support team to renew or reactivate the system subscription for <strong>{schoolName || 'Primary School ERP'}</strong>.
          </p>
          ...
          <div className="space-y-3">
            ...
            {/* Support Email */}
            <a
              href={`mailto:${payout.contactEmail || 'billing@edtechsolutions.co.ke'}?subject=Subscription Renewal for ${schoolName || 'Primary School ERP'}`}
      ...
      >
        <form onSubmit={handleSystemOwnerLogin} className="space-y-4 text-xs">
          <p className="text-slate-500">
            Enter the provider master authorization key to unlock and manage the Primary School ERP instance.
          </p>
          ...
          <div className="pt-2 flex items-center justify-between gap-2">
            <button
              ...
              onClick={async () => {
                try {
                  const res = await subscriptionService.restoreDefaultMasterPasskey(schoolName || 'SCHOOLM');
