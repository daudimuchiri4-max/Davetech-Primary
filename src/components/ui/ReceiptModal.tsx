      ...
      <div id="printable-receipt" className="p-6 bg-white border border-slate-200 rounded-xl space-y-6 text-slate-800">
        ...
        <div className="text-center border-b border-slate-200 pb-4">
          <div className="flex justify-center mb-3">
            <img
              src={(school?.logoUrl && !school.logoUrl.includes('unsplash.com')) ? school.logoUrl : '/school_logo.svg'}
              alt={school?.name || 'Primary School ERP'}
              className="w-24 h-24 object-contain drop-shadow-sm"
              referrerPolicy="no-referrer"
              onError={(e) => {
                e.currentTarget.src = '/school_logo.svg';
              }}
            />
          </div>
          ...
          <p className="text-xs text-slate-600 mt-1">{school?.address || 'Mariru Park, Kenya'} • Tel: {school?.phone || '+254 700 000 000'}</p>
