...
export const studentService = {
   ...
   * Format: SCHOOL/2026/001, SCHOOL/2026/002, etc.
   */
  formatNextAdmissionNumber(
    ...
    let maxSeq = 0;
    const cleanPrefix = (prefix || 'SCHOOL').trim().toUpperCase();
  ...
  async generateNextAdmissionNumber(
    ...
    try {
      ...
      return `${prefix || 'SCHOOL'}/${academicYear || '2026'}/001`;
    }
  },
  ...
  async createStudent(schoolId: string, studentData: Omit<Student, 'id' | 'schoolId' | 'createdAt' | 'updatedAt'>): Promise<Student> {
    ...
    if (!admNumber) {
      admNumber = await this.generateNextAdmissionNumber(schoolId, 'SCHOOL', '2026');
    }
