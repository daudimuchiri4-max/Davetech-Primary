...
class PrinterService {
  ...
  public buildEscPosReceipt(data: ThermalReceiptData, school: School | null): Uint8Array {
    ...
    appendLine(school?.name || 'Primary School ERP');
  ...
  public async printAllStudentIDCards(students: Student[], school: School | null): Promise<void> {
    ...
    const schoolLogo = (school?.logoUrl && !school.logoUrl.includes('unsplash.com')) ? school.logoUrl : '/school_logo.svg';
        ...
        <div class="id-card">
          <div class="card-header">
            ...
            <div class="school-info">
              <div class="school-name">${school?.name || 'Primary School ERP'}</div>
    ...
    const html = `
      ...
      <body>
        <div style="text-align: center; margin-bottom: 12px; font-size: 11px; font-weight: bold; color: #0f172a;">
          ${school?.name || 'Primary School ERP'} — Portrait Student ID Cards (${students.length} Cards)
        </div>
  ...
  public async runTestPrint(type: 'THERMAL_80' | 'THERMAL_58' | 'A4_RECEIPT' | 'A4_REPORT' | 'ID_CARD' | 'DRAWER', school: School | null): Promise<boolean> {
    ...
    if (type === 'THERMAL_80' || type === 'THERMAL_58') {
      const testData: ThermalReceiptData = {
        ...
        admissionNumber: 'SCHOOLM/2026/001',
      ...
      <body>
        <div class="text-center">
          <div style="margin-bottom: 6px;">
            <img src="${(school?.logoUrl && !school.logoUrl.includes('unsplash.com')) ? school.logoUrl : '/school_logo.svg'}" alt="Logo" style="max-height: ${is58 ? '42px' : '56px'}; max-width: 80%; object-fit: contain;" />
          </div>
          <div class="title">${school?.name || 'Primary School ERP'}</div>
          <div class="subtitle" style="font-weight: bold; color: #333;">${school?.motto || '— I can! I will! —'}</div>
          <div class="subtitle">${school?.address || 'Mariru Park, Kenya'}</div>
          <div class="subtitle">Tel: ${school?.phone || '+254 700 000 000'}</div>
      ...
      <body>
        <div class="header">
          <div style="display: flex; align-items: center; gap: 16px;">
            <img src="${(school?.logoUrl && !school.logoUrl.includes('unsplash.com')) ? school.logoUrl : '/school_logo.svg'}" alt="School Crest" style="width: 76px; height: 76px; object-fit: contain; padding: 2px;" />
            <div>
              <div class="school-name">${school?.name || 'Primary School ERP'}</div>
              <div class="school-sub" style="font-weight: bold; color: #ea580c;">${school?.motto || '— I can! I will! —'}</div>
              <div class="school-sub">${school?.address || 'Mariru Park, Kenya'} • Tel: ${school?.phone || '+254 700 000 000'} • Email: ${school?.email || 'admissions@example-school.ac.ke'}</div>
            </div>
          </div>
      ...
      <body>
        <div class="header">
          <div style="display: flex; align-items: center; gap: 14px;">
            <img src="${(school?.logoUrl && !school.logoUrl.includes('unsplash.com')) ? school.logoUrl : '/school_logo.svg'}" alt="School Crest" style="width: 65px; height: 65px; object-fit: contain; padding: 2px;" />
            <div>
              <div class="school-title">${school?.name || 'Primary School ERP'}</div>
              <div class="school-meta" style="font-weight: bold; color: #ea580c;">${school?.motto || '— I can! I will! —'}</div>
              <div class="school-meta">${school?.address || 'Mariru Park, Kenya'} • Tel: ${school?.phone || '+254 700 000 000'} • Email: ${school?.email || 'admissions@example-school.ac.ke'}</div>
            </div>
          </div>
          <div style="text-align: right;">
        ...
        <div class="card-container">
          ...
          <div class="id-card card-back">
            ...
            <div style="padding: 8px 12px; font-size: 7.5px; color: #334155; line-height: 1.4;">
              <p>1. This card is the property of <strong>${school?.name || 'Primary School ERP'}</strong>. If found, please return to the administration.</p>
        ...
        <div class="payment-channels">
          ...
          <div class="channels-grid">
            ...
            <div class="channel-card">
              ...
              <div class="channel-detail">Account Name: <strong>${school?.name || 'Primary School ERP'}</strong></div>
              <div class="channel-detail">Account No: <strong>${school?.paymentSettings?.bankAccountNumber || '000000000000'}</strong></div>
            </div>
          </div>
        </div>
      ...
      <body>
        <div class="header">
          <div style="display: flex; align-items: center; gap: 12px;">
            <img src="${(school?.logoUrl && !school.logoUrl.includes('unsplash.com')) ? school.logoUrl : '/school_logo.svg'}" alt="School Crest" style="width: 55px; height: 55px; object-fit: contain; padding: 2px;" />
            <div>
              <div class="school-title">${school?.name || 'Primary School ERP'}</div>
              <div class="school-sub" style="font-weight: bold; color: #ea580c;">${school?.motto || '— I can! I will! —'}</div>
              <div class="school-sub">${school?.address || 'Mariru Park, Kenya'} • Tel: ${school?.phone || '+254 700 000 000'}</div>
            </div>
          </div>
          <div style="text-align: right;">
        ...
        <div class="payment-channels">
          ...
          <div class="channels-grid">
            ...
            <div class="channel-card">
              ...
              <div style="font-size: 10.5px;">Account Name: <strong>${school?.name || 'Primary School ERP'}</strong></div>
              <div style="font-size: 10.5px;">Account No: <strong>${school?.paymentSettings?.bankAccountNumber || '000000000000'}</strong> • Nyahururu Branch</div>
            </div>
          </div>
        </div>
        ...
        <div class="footer-box">
          <div>
            <div style="font-size: 10.5px; color: #475569;">Bursar & Finance Office • ${school?.name || 'Primary School ERP'}</div>
  ...
  public printUserCredentialSlip(
    ...
    const schoolName = school?.name || 'Primary School ERP';
  ...
  public printDailyAttendanceReport(reportData: DailyAttendanceReportData, school: School | null): void {
    ...
    const schoolPhone = school?.phone || '+254 700 000 000';
    const schoolEmail = school?.email || 'admin@example-school.ac.ke';
