...
const getInitialView = (): ActiveView => {
  ...
  try {
    const savedUser = localStorage.getItem('school_erp_user');
    const savedView = localStorage.getItem('school_erp_active_view');
...
const MainAppContent: React.FC = () => {
  ...
  React.useEffect(() => {
    try {
      localStorage.setItem('school_erp_active_view', activeView);
    } catch {}
  }, [activeView]);
  ...
  const handleNavigate = (view: string) => {
    ...
    try {
      localStorage.setItem('school_erp_active_view', normalized);
